...
函数式编程是一种编程规范.在函数式编程中,是不允许使用变量的,对于一个函数只有一个确定的输入,因此也就有确定的输出.输入和输出可以是一个函数

#### 高阶函数

函数的形参是函数的函数被称为高阶函数

在高阶函数中,变量名可以指向一个函数名,也可以说函数名就是一个变量名,我们可以通过将函数名赋给一个变量,通过这个变量来调用该函数
...
map函数是一个高阶函数.map函数有两个参数,分别是一个函数和一个Iterable,对Iterable中的每个元素分别施加该函数的作用,然后将结果作为新的Iterator返回
...
reduce函数是一个高阶函数.reduce函数有两个参数,分别是一个函数f和一个Iterable,按顺序对Iterable的结果作累积运算:reduce(f,[x1,x2,x3,x4,x5]) <==> f(f(f(f(x1,x2),x3),x4),x5)
...
filter是一个过滤器,filter函数也有两个参数,分别是一个函数f和一个Iterable.f的返回值是bool,对Iterable中的每个元素施加f函数的作用,将返回true的元素保留,返回false的元素丢弃